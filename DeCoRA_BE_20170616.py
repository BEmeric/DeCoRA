...
def CNV_hypothesis():
	...
	# New Barcode is added for patient identification : IonCode_0116
        if ("IonDx" in file_sample):
	    # indice_Iondx=file_sample.split('_').index('IonDx')
            sample = file_sample.split('_')[1]
        elif ("IonXpress" in file_sample):
            indice_IonXpress=file_sample.split('_').index('IonXpress')
            sample = file_sample.split('_')[indice_IonXpress]+"_"+file_sample.split('_')[indice_IonXpress+1]
        elif ("IonCode_" in file_sample):
            indice_IonCode=file_sample.split('_').index('IonCode')
            sample = file_sample.split('_')[indice_IonCode]+"_"+file_sample.split('_')[indice_IonCode+1]
        else:
        ...
        for exon in values[filename]:
            if float(values[filename][exon][2]) > float(median_of_max_depths[exon]*2) and float(values[filename][exon][1]) > float(median_of_average_depths[exon]*3): 
                ...
                if ("IonDx-" in filename):
                    ...
                    indice_IonXpress=filename.split('/')[-1].split('_').index('IonXpress')
                    sample = "_".join(filename.split('/')[-1].split('_')[0:indice_IonXpress])+"\t"+filename.split('/')[-1].split('_')[indice_IonXpress]+"_"+filename.split('/')[-1].split('_')[indice_IonXpress+1]
                elif ("IonCode" in filename):
                    indice_IonCode=filename.split('/')[-1].split('_').index('IonCode')
                    sample = "_".join(filename.split('/')[-1].split('_')[0:indice_IonCode])+"\t"+filename.split('/')[-1].split('_')[indice_IonCode]+"_"+filename.split('/')[-1].split('_')[indice_IonCode+1]
                else:
