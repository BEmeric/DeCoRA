...
def generating_bed_with_introns():
    print "generating bed with introns"
...
def CNV_hypothesis():
    os.system('mkdir '+globalvar.INPUT_DIRECTORY+'/rename_dir/')
    os.system('cp '+globalvar.INPUT_DIRECTORY+'/*.bam'+' '+globalvar.INPUT_DIRECTORY+'/rename_dir/')

    # foreign bam: this option is desactivated and can be activated if we have foreign data
    # nb_bam_foreign = os.popen('ls '+globalvar.INPUT_DIRECTORY+'/rename_dir/*_rawlib.realigned.bam | wc -l')
    # if nb_bam_foreign != 0:
    #     os.system('rename s/_rawlib.realigned// '+globalvar.INPUT_DIRECTORY+'/rename_dir/*_rawlib.realigned.bam')

    # Identify bam file IonPgm format
    nb_bam_pgm=os.popen('ls '+globalvar.INPUT_DIRECTORY+'rename_dir/*_rawlib.bam | wc -l')
    bam_pgm=nb_bam_pgm.readline().strip('\n')
    # print bam_pgm

    if str(bam_pgm) != "0":
        print "Bam file from Ion Pgm detected ..."
        os.system('rename s/_rawlib// '+globalvar.INPUT_DIRECTORY+'/rename_dir/*_rawlib.bam')

    # Identify bam file NextSeq Illumina format
    nb_bam_NextSeqIllumina=os.popen('ls '+globalvar.INPUT_DIRECTORY+'/rename_dir/*_S1.bam | wc -l')
    bam_NextSeqIllumina=nb_bam_NextSeqIllumina.readline().strip('\n')
    # print bam_NextSeqIllumina

    if str(bam_NextSeqIllumina) != "0":
        print "Bam file from Illumina NextSeq detected ..."
        os.system('rename s/_S1// '+globalvar.INPUT_DIRECTORY+'/rename_dir/*_S1.bam')

    # In the case of foreign bam data 
    ...
    for filename in filenames[:-1]:
        ...
        sample=check_file_format_name(file_sample)
        ind = Sample(sample)

        bam_extension = ""
        if globalvar.VERBEUX:
            print ('Reading of...'+sample+'_analysis.txt...')
        nb_reads = os.popen('samtools view '+globalvar.INPUT_DIRECTORY+'/rename_dir/'+sample+'.bam | wc -l').read().split('\n')[0]

        values[filename] = {}
        IN = open(filename, 'r')
        # We have 8 line in the header of results file.split('\n')
        line=IN.readline()
        line=IN.readline()
        line=IN.readline()
        line=IN.readline()
    ...
    report_name, RunName, TeamName = ind.get_TeamName_RunName_ReportName()
    ...
    for filename in values:
        sample=check_file_format_name(filename)
...
def Compute_no_covered_exon():
    ...
    for filename in filenames[:-1]:
        ...
        sample=check_file_format_name(file_sample)
        ind = Sample(sample)
        num_patient = file_sample.split('_')[0]

        if globalvar.VERBEUX:
            print "starting for "+num_patient+"_"+sample
        IN = open(filename, 'r')
        ...
        IN.close()
        print "end for "+num_patient+"_"+sample
    ...
    RES = open(globalvar.OUTPUT_DIRECTORY+'No_covered_exons_summary.txt', 'w')       
    # Write header info
    RES.write("Equipe : \t"+TeamName+"\n")
    RES.write("Run : \t"+RunName+"\n")
    RES.write("Report Name : \t"+report_name+"\n")
    ...
    RES.close()
    if globalvar.VERBEUX:
        print "End of Computing for no coverage exons"

# ------------------------------------------ Check bam format name  -------------------------------------
def check_file_format_name(filename):
    # modif Eméric
    # modif line : 2 barcodes type for Pgm life (Example: IonDx-2 and IonXpress_002)
    # modif line : format donnees illumina NextSeq (16ag040-TruSeqRapidExome_S1.bam)
    # Modif line : new barcode was added Example:'IonCode_0101'
    if ("IonDx" in filename) and ( ("_report.txt" in filename) or ("_analysis.txt" in filename) ):
        sample = filename.split('_')[1]
    elif ("IonXpress" in filename) and ( ("_report.txt" in filename) or ("_analysis.txt" in filename) ):
        sample = filename.split('_')[1]+"_"+filename.split('_')[2]
    elif ("IonCode" in filename) and ( ("_report.txt" in filename) or ("_analysis.txt" in filename) ):
        sample = filename.split('_')[1]+"_"+filename.split('_')[2]
    # For Pgm bam file
    elif ("IonDx" in filename) and ( "bam" in filename ):
        sample = filename.split('_')[0]
    elif ("IonXpress" in filename) and ("bam" in filename):
        sample = filename.split('_')[0]+"_"+filename.split('_')[1]
    elif ("IonCode" in filename) and ("bam" in filename):
        sample = filename.split('_')[0]+"_"+filename.split('_')[1]
    # For Illumina bam file
    elif ("_S1" in filename) and ("bam" in filename):
        sample = filename.split('.')[0]
    else:
        print "barcode name format not detected, so the whole name of bam file is taken as barcode please check it anyway!"
        sample = filename.split('.')[0]
        # sample = "None"
    return sample
...
# -------------------------------------------------- FOR MULTI-THREADING --------------------------------------------------
        ...
        os.system('rm -rf '+globalvar.INPUT_DIRECTORY+'/rename_dir/')
